...
public class WhUserTypeUtil {
	...
	public void generatePieChart(String location, List<Object[]> list) {
		...
		JFreeChart chart = ChartFactory.createPieChart3D("Wh User Type", dataset);

		//// c. Convert JFreeChart object into one Image using ChartUtil class
		try {
			ChartUtils.saveChartAsJPEG(new File(location + "/whusertypeA.jpg"), chart, 400, 300);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void generateBarChart(String location, List<Object[]> list) {
		...
		//// c. Convert JFreeChart object into one Image using ChartUtil class
		try {
			ChartUtils.saveChartAsJPEG(new File(location + "/whusertypeB.jpg"), chart, 500, 340);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
