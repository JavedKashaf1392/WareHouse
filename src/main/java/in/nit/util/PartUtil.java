...
public class PartUtil {
	...
	public void generatePieChart(String location, List<Object[]> list) {
		...
		JFreeChart chart = ChartFactory.createPieChart3D("Base Currency", dataset);

		//// c. Convert JFreeChart object into one Image using ChartUtil class
		try {
			ChartUtils.saveChartAsJPEG(new File(location + "/partA.jpg"), chart, 400, 300);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void generateBarChart(String location, List<Object[]> list) {
		...
		// b. Convert DataSet data into JFreeChart object using ChartFactory class
		JFreeChart chart = ChartFactory.createBarChart("Base Currency", "CURRENCY", "COUNT", dataset);

		//// c. Convert JFreeChart object into one Image using ChartUtil class
		try {
			ChartUtils.saveChartAsJPEG(new File(location + "/partB.jpg"), chart, 500, 340);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
